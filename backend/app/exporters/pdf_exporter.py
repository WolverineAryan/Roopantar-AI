...
def export_to_pdf(format_id: str, content: Dict[str, Any], output_path: Path) -> Path:
    ...
    meta_style = ParagraphStyle(
        ...
        fontSize=9,
        leading=12,
        textColor=colors.HexColor('#DC2626') if content.get('severity', '').lower() in ['high', 'critical'] else colors.HexColor('#2563EB')
    )
    
    story = []
    
    title_text = content.get("title", content.get("infographic_title", content.get("video_title", "Roopantar-AI Report")))
    ...
    if format_id == "advisory":
        ...
        story.append(Paragraph(f"ID: {adv_id} | DATE: {date} | SEVERITY: {sev.upper()}", meta_style))
    ...
    if format_id == "advisory":
        ...
        story.append(Paragraph(content.get("impact_assessment", ""), body_style))
        ...
        for action in content.get("recommended_actions", []):
            table_data.append([
                action.get("priority", "Immediate"),
                action.get("target_team", "Ops"),
                action.get("action", "")
            ])
        ...
        story.append(Paragraph(content.get("strategic_context", ""), body_style))
        
        story.append(Paragraph("2. Bottom Line Up Front (BLUF)", h1_style))
        story.append(Paragraph(f"<b>{content.get('bottom_line_up_front', '')}</b>", body_style))
    ...
    story.append(Paragraph("Roopantar-AI Automated Content Transformation | NTRO SIH26154", ParagraphStyle('Foot', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#94A3B8'))))
